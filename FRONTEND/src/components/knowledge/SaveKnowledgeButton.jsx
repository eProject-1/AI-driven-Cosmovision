...
function hasStoredSession() {
  ...
  return Boolean(window.localStorage.getItem("cosmovision_token"));
}

export function SaveKnowledgeButton({ slug, title, category, imageUrl, imageSrc }) {
  const { user } = useAuth();

  const safeSlug = slug ? String(slug) : "";
  const isLoggedIn = Boolean(user?.id || hasStoredSession());
  ...
  const onClick = async () => {
    ...
    try {
      if (!isSaved) {
        await saveCosmicKnowledge(safeSlug, { title, category, imageUrl, imageSrc });
        setIsSaved(true);
      } else {
  ...
  const label = needsSignIn || !isLoggedIn ? 'Sign in to save' : isSaved ? 'Saved' : 'Save';
