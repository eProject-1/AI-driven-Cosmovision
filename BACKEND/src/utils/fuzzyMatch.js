...
/**
 * Trả về danh sách CÁC KEYWORD THỰC SỰ khớp với message (không phải true/false).
 * Đây là nguồn logic DUY NHẤT cho việc so khớp — hasKeyword() chỉ là wrapper
 * của hàm này, để tránh hai nơi cài hai thuật toán lệch nhau (như bug đã gặp).
 */
export function findMatchedKeywords(normalizedMessage, keywords, fuzzyThreshold = 0.78) {
  ...
  return keywords.filter(keyword => {
    if (message.includes(keyword)) return true; // exact substring - đường nhanh
